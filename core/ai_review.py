...
def divergence_review(
    ...
    *,
    ...
    """对持仓的 MACD 背驰预警做一次自然语言解读（结合 EMA 量能与相对强度）。"""
    system = _GUARDRAIL + (
        " 你精通 MACD 与缠论「背驰」分析，理解顶背驰(涨势衰竭)、底背驰(跌势衰竭) 的反转预警含义，"
        "能结合 EMA 量能趋势与相对强度，判断哪些持仓动能正在见顶转弱、哪些正在见底企稳，"
        "并说明背驰信号与量能/相对强度是否共振。"
    )
    user = (
        "下面是一支半导体组合中**触发 MACD 背驰预警**的持仓（我们已用近半年日线自算好 "
        "MACD 与价格枢轴，判定顶/底背驰，并附 EMA 量能分与相对强度做共振参考）。\n"
        "请做一次中文预警解读，帮基金经理回答：**哪些顶背驰需警惕见顶、哪些底背驰值得关注见底、"
        "背驰与量能/相对强度是否共振**，并结合仓位占比点出轻重。\n\n"
        f"数据：\n{json.dumps(payload, ensure_ascii=False, indent=2)}\n\n"
        "字段说明：'顶背驰'=价创新高但 MACD 动能走弱(涨势或衰竭，偏空预警)；"
        "'底背驰'=价创新低但 MACD 动能转强(跌势或衰竭，偏多关注)；"
        "'量能分'越高趋势越强(🟢≥70/🟡40-69/🔴<40)；'相对强度'为相对 SOXX 的强弱(领涨/同步/落后)；"
        "顶背驰+量能转弱+相对落后=偏空共振，底背驰+相对领涨=偏多共振，背驰是早期预警需结合确认。\n\n"
        "请严格输出如下 JSON：\n"
        "{\n"
        '  "overview": "3~5 句总览：组合当前有哪些顶/底背驰、整体动能见顶还是见底（结合占比）",\n'
        '  "top_div": [{"ticker": "XX", "note": "为何顶背驰需警惕、量能/相对强度是否印证"}],\n'
        '  "bottom_div": [{"ticker": "XX", "note": "为何底背驰值得关注、是否有企稳迹象"}],\n'
        '  "actions": ["可考虑关注的方向或动作（中性表述，非指令）…"],\n'
        '  "risks": ["需警惕的风险点（如高仓位却顶背驰、背驰失败继续单边）…"]\n'
        "}\n"
        "背驰是概率性早期信号，务必提醒需价格/量能确认，不可单独据此操作；"
        "top_div/bottom_div 从数据中选取代表性标的即可，优先点名仓位占比大的持仓。"
        "数据不足时相应字段给空数组。"
    )
    return llm.chat_json(system, user, model=model, max_tokens=1600)
